...
class StoryState extends State<Story> {
  ...
  void initState() {
    super.initState();
    if (widget.cat != null) getBook();
  }

  getBook() async {
    await Firestore.instance.collection('categories').document(widget.cat)
        .collection('books').document(widget.book_id).get()
        .then((value) {
      setState(() {
        book = value.data;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: widget.cat != null
          ...
          child: book==null?LinearProgressIndicator():Html(
            data: book['story'],
            style: {
              "body": Style(
